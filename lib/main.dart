...
Future<void> main() async {
  ...
  AdsManager.createInterstitialAd(adsId: AdsManager.getFavouritePageInterstitialAdUnitId()!);
  runApp(
...
class MainPage extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    return MaterialApp(
      ...
      home: sliderValue ? OnBoardingScreen() : MyHomePage(selectedIndex: 2,info: "home",),
    );
  }
}
...
class MyHomePage extends StatefulWidget {
  ...
  final String info;
  MyHomePage({Key? key, this.title, required this.selectedIndex, required this.info}) : super(key: key);
    ...
    if(uri!= null){
      if(uri.host == "categoryid") {
        Navigator.of(context).pushAndRemoveUntil(
          MaterialPageRoute(builder: (context)=>MyHomePage(selectedIndex: 1,info: "widget")), 
          (route) => false);
        
    AdsManager.createInterstitialAd(adsId: AdsManager.getWidgetPressedInterstitialAdUnitId()!);
    AdsManager.showInterstitialAd(adsId: AdsManager.getWidgetPressedInterstitialAdUnitId()!);
    }
      ...
      if(aya!=null) {
        ...
        Navigator.of(context).pushAndRemoveUntil(
          MaterialPageRoute(builder: (context)=>MyHomePage(selectedIndex: 1,info: "widget",)), (route) => false);
        Navigator.of(context).push(MaterialPageRoute(builder:(context)=> CategoryManager(
          category: category,
      ...
      final categoryId = await HomeWidget.getWidgetData('categoryId');
      final index = await HomeWidget.getWidgetData("index");
      var favCatManager = Provider.of<FavCategoryManager>(context,listen: false);
      final aya = await HomeWidget.getWidgetData('favourite');
      if(aya!=null){
      List<AddAyaModel> ayaList = AddAyaModel.decode(aya);
      bool isFav = getWallpapersOfCurrentCategory(ayaList[0],context,false);
      if (isFav) {
        favCatManager.removeFromFav(ayaList[0]);
        ScaffoldMessenger.of(context).showSnackBar(snackBar("إزالتها من المفضلة"));
      } else {
